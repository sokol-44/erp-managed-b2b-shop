...
-- Zrzut struktury b_func_client_user_set_address
DELIMITER //
CREATE  ` FUNCTION `b_func_client_user_set_address`(`id_address_in` INT, `id_client_user_in` INT, `id_client_in` INT, `description_in` TEXT, `name_in` TEXT, `street_in` TEXT, `city_in` TEXT, `zip_code_in` TEXT, `country_in` TEXT, `state_in` TEXT) RETURNS text CHARSET latin2
    MODIFIES SQL DATA
BEGIN
	...
	ELSE
      ...
      IF status_client_user_address IS NULL THEN
         ...
         description, name, street, city, zip_code, country, date_created, state) values 
         (id_address_in, id_client_user_in, id_client_in, 
         description_in, name_in, street_in, city_in, zip_code_in, country_in, now(), state_in);
      ...
      ELSE
         ...
         zip_code = zip_code_in, country = country_in, date_modified = now(), state = state_in
