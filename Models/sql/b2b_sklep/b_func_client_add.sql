...
# Dumping structure for function b2b_sklep.b_func_client_add
DROP FUNCTION IF EXISTS `b_func_client_add`;
DELIMITER //
CREATE DEFINER=`root`@`localhost` FUNCTION `b_func_client_add`(`id_client_in` INT, `name_in` TINYTEXT, `description_in` TEXT, `email_in` TINYTEXT, `phone_in` TINYTEXT, `state_in` TINYTEXT) RETURNS tinytext CHARSET utf8
    READS SQL DATA
BEGIN
