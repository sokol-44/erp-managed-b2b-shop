...
-- Zrzut struktury procedura company_22.b_proc_order_invoice_set
DELIMITER //
CREATE  ` PROCEDURE `b_proc_order_invoice_set`(`id_invoice_in` INT, `id_order_in` INT, `id_client_in` INT, `invoice_number_in`  VARCHAR(50), `state_in` VARCHAR(50), `net_value_in` DECIMAL(10,2), `gross_value_in` DECIMAL(10,2), `date_issue_in` DATE, `date_pay_in` DATE,  `description_in` TEXT)
BEGIN
