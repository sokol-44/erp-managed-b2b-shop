...
-- Zrzut struktury procedura company_1.b_proc_client_user_add
DROP PROCEDURE IF EXISTS `b_proc_client_user_add`;
DELIMITER //
CREATE DEFINER=`company_1`@`%` PROCEDURE `b_proc_client_user_add`(IN `id_client_user_in` INT, IN `id_client_in` INT, IN `login_in` tinytext, IN `name_in` TINYTEXT, IN `description_in` TEXT, IN `email_in` TINYTEXT, IN `phone_in` TINYTEXT, IN `phone_cell_in` TINYTEXT, IN `state_in` TINYTEXT)
BEGIN
