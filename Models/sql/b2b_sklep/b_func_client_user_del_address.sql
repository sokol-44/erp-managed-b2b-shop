...
-- Zrzut struktury funkcja company_20.b_func_client_user_del_address
DELIMITER //
CREATE DEFINER=`company_20`@`%` FUNCTION `b_func_client_user_del_address`(`id_address_in` INT, `id_client_user_in` INT, `id_client_in` INT) RETURNS text CHARSET latin2
    MODIFIES SQL DATA
BEGIN
