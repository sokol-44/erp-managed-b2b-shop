...
-- Zrzut struktury funkcja company_1.b_func_client_user_del_address
DROP FUNCTION IF EXISTS `b_func_client_user_del_address`;
DELIMITER //
CREATE DEFINER=`company_1`@`%` FUNCTION `b_func_client_user_del_address`(`id_address_in` INT, `id_client_user_in` INT, `id_client_in` INT) RETURNS text CHARSET latin2
    MODIFIES SQL DATA
BEGIN
