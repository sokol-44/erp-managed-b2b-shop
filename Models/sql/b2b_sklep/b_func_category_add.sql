...
-- Zrzut struktury funkcja b2b_sklep.b_func_category_add
DROP FUNCTION IF EXISTS `b_func_category_add`;
DELIMITER //
CREATE DEFINER=`root`@`localhost` FUNCTION `b_func_category_add`(`id_category_in` INT, `id_category_parent_in` INT, `sort_order_in` INT, `root_number_in` TINYINT, `name_in` TEXT, `description_in` TEXT) RETURNS tinytext CHARSET utf8
    READS SQL DATA
BEGIN
	...
	IF status_category IS NULL THEN
		...
		SET status = CONCAT(@status_proc,',CATEGORY_NEW');
	ELSE
		SET status = 'ERROR,CATEGORY_EXIST';
	END IF;
	return status;
END//
DELIMITER ;
