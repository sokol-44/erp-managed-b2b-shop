...
-- Zrzut struktury funkcja company_20.b_func_account_manager_set
DELIMITER //
CREATE DEFINER=`company_20`@`%` FUNCTION `b_func_account_manager_set`(`id_account_manager_in` INT, `id_client_in` INT, `id_client_user_in` INT, `account_manager_name_in` VARCHAR(10), `fullname_in` TINYTEXT, `phone1_in` TINYTEXT, `phone2_in` TINYTEXT,  `email_in` TINYTEXT) RETURNS tinytext CHARSET latin2
    READS SQL DATA
BEGIN
