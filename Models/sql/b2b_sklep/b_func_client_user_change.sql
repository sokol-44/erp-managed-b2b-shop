...
-- Zrzut struktury funkcja company_1.b_func_client_user_change
DROP FUNCTION IF EXISTS `b_func_client_user_change`;
DELIMITER //
CREATE DEFINER=`company_1`@`%` FUNCTION `b_func_client_user_change`(`id_client_user_in` INT, `id_client_in` INT, `login_in` TINYTEXT, `name_in` TINYTEXT, `description_in` TEXT, `email_in` TINYTEXT, `phone_in` TINYTEXT, `phone_cell_in` TINYTEXT, `state_in` TINYTEXT) RETURNS tinytext CHARSET latin2
    READS SQL DATA
BEGIN
