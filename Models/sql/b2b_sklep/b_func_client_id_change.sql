...
DELIMITER //
CREATE DEFINER=`company_20`@`%` FUNCTION `b_func_client_id_change`(`id_client_in` INT, `id_client_new` INT) RETURNS tinytext CHARSET latin2
    READS SQL DATA
BEGIN
	...
	ELSE		
		...
		WHERE `id_client`=id_client_in AND `state`= "NEW";
