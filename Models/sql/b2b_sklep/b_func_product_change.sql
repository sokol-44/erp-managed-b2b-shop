...
# Dumping structure for function b2b_sklep.b_func_product_change
DROP FUNCTION IF EXISTS `b_func_product_change`;
DELIMITER //
CREATE DEFINER=`root`@`localhost` FUNCTION `b_func_product_change`(`id_product_in` INT, `name_in` TINYTEXT, `description_in` TEXT, `picture_small_url_in` TEXT, `picture_big_url_in` TEXT, `price_in` DECIMAL, `vat_in` DECIMAL, `quantity_in` INT, `status_in` TEXT) RETURNS tinytext CHARSET utf8
    READS SQL DATA
BEGIN
