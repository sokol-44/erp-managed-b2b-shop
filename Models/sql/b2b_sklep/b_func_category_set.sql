...
-- Zrzut struktury funkcja company_20.b_func_category_set
DELIMITER //
CREATE DEFINER=`company_20`@`%` FUNCTION `b_func_category_set`(`id_category_in` INT, `id_category_parent_in` INT, `sort_order_in` INT, `root_number_in` TINYINT, `name_in` TEXT, `description_in` TEXT) RETURNS tinytext CHARSET utf8
    READS SQL DATA
BEGIN
