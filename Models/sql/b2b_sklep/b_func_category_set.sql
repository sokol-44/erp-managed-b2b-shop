...
-- Zrzut struktury funkcja b2b_sklep.b_func_category_set
DROP FUNCTION IF EXISTS `b_func_category_set`;
DELIMITER //
CREATE DEFINER=`root`@`localhost` FUNCTION `b_func_category_set`(`id_category_in` INT, `id_category_parent_in` INT, `sort_order_in` INT, `root_number_in` TINYINT, `name_in` TEXT, `description_in` TEXT) RETURNS tinytext CHARSET utf8
    READS SQL DATA
BEGIN
