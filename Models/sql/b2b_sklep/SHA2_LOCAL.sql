...
-- Zrzut struktury funkcja b2b_sklep.SHA2_LOCAL
DROP FUNCTION IF EXISTS `SHA2_LOCAL`;
DELIMITER //
CREATE DEFINER=`root`@`localhost` FUNCTION `SHA2_LOCAL`(message MEDIUMBLOB, bits SMALLINT) RETURNS char(64) CHARSET utf8
    DETERMINISTIC
BEGIN
    DECLARE k, w BLOB;
