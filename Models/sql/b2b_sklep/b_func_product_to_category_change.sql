...
# Dumping structure for function b2b_sklep.b_func_product_to_category_change
DROP FUNCTION IF EXISTS `b_func_product_to_category_change`;
DELIMITER //
CREATE DEFINER=`root`@`localhost` FUNCTION `b_func_product_to_category_change`(`id_category_in` INT, `id_product_in` INT) RETURNS tinytext CHARSET utf8
    READS SQL DATA
BEGIN
