...
-- Zrzut struktury funkcja company_1.b_func_client_change
DROP FUNCTION IF EXISTS `b_func_client_change`;
DELIMITER //
CREATE DEFINER=`company_1`@`%` FUNCTION `b_func_client_change`(`id_client_in` INT, `name_in` TINYTEXT, `description_in` TEXT, `email_in` TINYTEXT, `phone_in` TINYTEXT, `state_in` TINYTEXT) RETURNS tinytext CHARSET utf8
    READS SQL DATA
BEGIN
