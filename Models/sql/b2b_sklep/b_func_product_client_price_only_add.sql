...
-- Zrzut struktury funkcja company_1.b_func_product_client_price_only_add
DROP FUNCTION IF EXISTS `b_func_product_client_price_only_add`;
DELIMITER //
CREATE DEFINER=`company_1`@`%` FUNCTION `b_func_product_client_price_only_add`(`id_client_in` INT, `id_product_in` INT, `price_in` DECIMAL(10,0), `vat_in` INT) RETURNS tinytext CHARSET utf8
    MODIFIES SQL DATA
    DETERMINISTIC
BEGIN
