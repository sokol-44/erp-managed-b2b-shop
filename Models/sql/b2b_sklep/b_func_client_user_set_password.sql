...
# Dumping structure for function b2b_sklep.b_func_client_user_set_password
DROP FUNCTION IF EXISTS `b_func_client_user_set_password`;
DELIMITER //
CREATE DEFINER=`root`@`localhost` FUNCTION `b_func_client_user_set_password`(`id_client_user_in` INT, `password_in` TINYTEXT, `password_salt_in` BLOB) RETURNS tinytext CHARSET utf8
    READS SQL DATA
BEGIN
