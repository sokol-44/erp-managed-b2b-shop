...
-- Zrzut struktury b_func_client_user_set_password
DELIMITER //
CREATE  ` FUNCTION `b_func_client_user_set_password`(`id_client_user_in` INT, `id_client_in` INT, `password_in` TINYTEXT, `password_salt_in` BLOB) RETURNS tinytext CHARSET latin2
    READS SQL DATA
BEGIN
