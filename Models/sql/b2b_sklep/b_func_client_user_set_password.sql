...
-- Zrzut struktury funkcja company_20.b_func_client_user_set_password
DELIMITER //
CREATE DEFINER=`company_20`@`%` FUNCTION `b_func_client_user_set_password`(`id_client_user_in` INT, `id_client_in` INT, `password_in` TINYTEXT, `password_salt_in` BLOB) RETURNS tinytext CHARSET latin2
    READS SQL DATA
BEGIN
