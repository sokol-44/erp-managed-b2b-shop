...
-- Zrzut struktury procedura company_1.b_proc_category_add
DROP PROCEDURE IF EXISTS `b_proc_category_add`;
DELIMITER //
CREATE DEFINER=`company_1`@`%` PROCEDURE `b_proc_category_add`(IN `id_category_in` INT, IN `id_category_parent_in` INT, IN `sort_order_in` INT, IN `root_number_in` TINYINT, IN `name_in` TEXT, IN `description_in` TEXT)
BEGIN
