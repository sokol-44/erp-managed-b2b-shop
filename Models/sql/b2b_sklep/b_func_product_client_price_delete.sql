...
-- Zrzut struktury funkcja company_1.b_func_product_client_price_delete
DROP FUNCTION IF EXISTS `b_func_product_client_price_delete`;
DELIMITER //
CREATE DEFINER=`company_1`@`%` FUNCTION `b_func_product_client_price_delete`(`id_client_in` INT, `id_product_in` INT) RETURNS tinytext CHARSET utf8
    MODIFIES SQL DATA
    DETERMINISTIC
BEGIN
