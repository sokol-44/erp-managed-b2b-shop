...
-- Zrzut struktury funkcja company_1.b_func_product_to_category_set_product_category
DROP FUNCTION IF EXISTS `b_func_product_to_category_set_product_category`;
DELIMITER //
CREATE DEFINER=`company_1`@`%` FUNCTION `b_func_product_to_category_set_product_category`(`id_category_in` INT, `id_product_in` INT) RETURNS tinytext CHARSET utf8
    READS SQL DATA
BEGIN
