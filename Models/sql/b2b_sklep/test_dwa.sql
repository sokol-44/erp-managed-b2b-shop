...
# Dumping structure for procedure b2b_sklep.test_dwa
DROP PROCEDURE IF EXISTS `test_dwa`;
DELIMITER //
CREATE DEFINER=`root`@`localhost` PROCEDURE `test_dwa`()
BEGIN
DECLARE stream BLOB(256) DEFAULT '';
DECLARE stream_pos INT DEFAULT 0;
DECLARE idx INT DEFAULT 0;
DECLARE idx_j INT DEFAULT 0;
DECLARE ckey BLOB(256) DEFAULT 'Wiki';
DECLARE ckey_lenght INT DEFAULT 0;
DECLARE ptext BLOB DEFAULT 'pedia';
DECLARE ptext_lenght INT DEFAULT 0;
DECLARE ptext_idx INT DEFAULT 0;
DECLARE temp BLOB default '';
DECLARE ret_stream BLOB default '';
	...
	REPEAT
		...
		SET stream = CONCAT( SUBSTRING( stream, 1, idx), SUBSTRING( stream, idx_j+1, 1) , SUBSTRING( stream, idx+2) );
		SET stream = CONCAT( SUBSTRING( stream, 1, idx_j), temp , SUBSTRING( stream, idx_j+2) );
		SET stream_pos = MOD( (ASCII( SUBSTRING( stream, idx+1, 1) ) + ASCII( SUBSTRING( stream, idx_j+1, 1) ) ), 256);
	...
	select hex(stream), LENGTH(stream), idx, hex(SUBSTRING( stream, 254)), hex(ret_stream) ;
