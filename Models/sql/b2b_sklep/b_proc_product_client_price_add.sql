...
-- Zrzut struktury procedura company_1.b_proc_product_client_price_add
DROP PROCEDURE IF EXISTS `b_proc_product_client_price_add`;
DELIMITER //
CREATE DEFINER=`company_1`@`%` PROCEDURE `b_proc_product_client_price_add`(IN `id_client_in` INT, IN `id_product_in` INT, IN `price_in` DECIMAL(10,0), IN `vat_in` INT)
BEGIN
