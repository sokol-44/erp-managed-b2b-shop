...
-- Zrzut struktury funkcja b2b_sklep.b_func_product_client_price_add
DROP FUNCTION IF EXISTS `b_func_product_client_price_add`;
DELIMITER //
CREATE DEFINER=`root`@`localhost` FUNCTION `b_func_product_client_price_add`(`id_product_in` INT, `id_client_in` INT, `price_in` DECIMAL(10,0), `vat_in` INT) RETURNS tinytext CHARSET utf8
    MODIFIES SQL DATA
    DETERMINISTIC
BEGIN
	...
	DECLARE `status_product_price` TINYTEXT DEFAULT NULL;
	...
	IF status_product IS NULL OR status_client IS NULL THEN
		SET status = CONCAT('ERROR', ',PRODUCT_OR_CLIENT_DONT_EXIST');
	ELSE
