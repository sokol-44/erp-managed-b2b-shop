...
-- Zrzut struktury funkcja company_1.b_func_category_change
DROP FUNCTION IF EXISTS `b_func_category_change`;
DELIMITER //
CREATE DEFINER=`company_1`@`%` FUNCTION `b_func_category_change`(`id_category_in` INT, `id_category_parent_in` INT, `sort_order_in` INT, `root_number_in` TINYINT, `name_in` TEXT, `description_in` TEXT) RETURNS tinytext CHARSET utf8
    READS SQL DATA
BEGIN
