...
foreach( $product_list as $product ) {
   ...
   $link_basket = $F->make_link(CFG_COM_BASKET, $F->add_local_get('mode', 'add_basked', $GET_tmp));
   $link_product_info = $F->make_link(CFG_COM_PRODUCT_INFO, $GET_tmp);
   $cell_basket = $F->draw_link($link_basket, 'onclick="add_basked()" title="' . Lang::_('add_basked') . '"', $F->static_image('icon/buy_16.png', Lang::_('Remove')));
